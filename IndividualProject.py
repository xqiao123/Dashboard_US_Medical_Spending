...
##Preparation
med=pd.read_csv('medical.csv', encoding='latin-1')
...
app.layout = html.Div([
    ...
    html.Div([html.H6('Introduction: ',
             ...
             html.Article("This is an interactive dashboard for the user to compare each state's Medical Spending Per Beneficiary (MSPB) in the contiguous US,\
              ...
              The user can change the plots or table results according to following options:",
                          style={'width':'100%'})]),
    ...
    html.Div(html.Article('-'*280), style={'color':'silver'}),
    ...
    html.Div([html.H6('My Findings: ',
                      style={'color':'navy',
                             'font-family':'serif'}),
          html.Article(" 1) When we don't adjust any dropdown or sliders, we can find that Maryland has the highest MSPB ($10,061), and MSPB is relatively high in the Midwest and East of the United States according to the map plot."),
...
def update_table(obamacare, number_range, number_range2):
    geo_df2 = geo_df[((geo_df.Smoking_Rate<=(max(number_range2))) & (geo_df.Smoking_Rate>=(min(number_range2)))) & (geo_df.Obamacare.isin(obamacare)) & ((geo_df.Unemployment_Rate<=(max(number_range)))&(geo_df.Unemployment_Rate>=(min(number_range))))].sort_values('MSPB', ascending=False)                       
    geo_df3=geo_df2[['STATE_ABBR','MSPB','Obamacare','Unemployment_Rate','Smoking_Rate']]
    geo_df4=geo_df3.rename(columns={'STATE_ABBR':'State','Unemployment_Rate':'Unemployment Rate','Smoking_Rate':'Smoking Rate'})
    
    return generate_table(geo_df4)
